...
def read_dicom(filename,ioption):
    ...
    with PdfPages(dirname + '/' + 'Light-rad_report.pdf') as pdf:
        ...
        # here would be more accurate to interpolate first

        xaxis = np.linspace(0,len(profilehorz)*dx,len(profilehorz))
        yaxis = np.linspace(0,len(profilevert)*dy,len(profilevert))

        xaxis_interp = np.linspace(xaxis[0],xaxis[-1],len(xaxis)*10, endpoint=True)
        yaxis_interp = np.linspace(yaxis[0],yaxis[-1],len(yaxis)*10, endpoint=True)

        width_interp=len(xaxis_interp)
        height_interp=len(yaxis_interp)



        profilehorz_interp=interp1d(xaxis,profilehorz,kind='linear')
        profilevert_interp=interp1d(yaxis,profilevert,kind='linear')

        top_edge,index_top = u.find_nearest(profilevert_interp(yaxis_interp)[0:height_interp//2], 0.5)
        bot_edge,index_bot = u.find_nearest(profilevert_interp(yaxis_interp)[height_interp//2:height_interp], 0.5)

        l_edge,index_l = u.find_nearest(profilehorz_interp(xaxis_interp)[0:width_interp//2], 0.5) #finding the edge of the field on the bottom
        r_edge,index_r = u.find_nearest(profilehorz_interp(xaxis_interp)[width_interp//2:width_interp], 0.5) #finding the edge of the field on the right
        ...
        ax.annotate(s='', xy=(PROFILE['vertical']*dx/10,index_top*dy/100 ), xytext=(PROFILE['vertical']*dx/10,(height//2+index_bot/10)*dy/10), arrowprops=dict(arrowstyle='<->',color='r')) # example on how to plot a double headed arrow
        ax.text((PROFILE['vertical'] + 10)*dx/10, (height // 2)*dy/10,'Vfs='+str(round((height//2+index_bot/10-index_top/10)*dy/10,2))+'cm',rotation=90, fontsize=14, color='r')

        #adding a horizontal arrow
        print(index_l*dx,index_l,PROFILE['horizontal']*dy,PROFILE['horizontal'])
        ax.annotate(s='', xy=(index_l*dx/100,PROFILE['horizontal']*dy/10), xytext=((width // 2 + index_r/10)*dx/10,PROFILE['horizontal']*dy/10),
                    arrowprops=dict(arrowstyle='<->',color='r'))  # example on how to plota double headed arrow
        ax.text((width//2)*dx/10, (PROFILE['horizontal']+10)*dy/10, 'Hfs='+str(round((width // 2 + index_r/10-index_l/10)*dx/10,2))+'cm', rotation=0, fontsize=14, color='r')
